...
function MatchRow({ match, showScore = false, isLive = false }) {
  const home = match.home || match.homeTeam?.shortName || '?'
  const away = match.away || match.awayTeam?.shortName || '?'
  const hFlag = FLAGS[home] || '🏳️'
  const aFlag = FLAGS[away] || '🏳️'
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', fontSize: '0.875rem', fontWeight: 600 }}>
        <span style={{ fontSize: '1.2rem' }}>{hFlag}</span>
        <span>{home}</span>
      </div>
      ...
      <div style={{ textAlign: 'center', minWidth: '5rem' }}>
        {showScore ? (
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.3rem', justifyContent: 'center' }}>
            ...
            <span style={{ fontFamily: 'var(--font-display)', fontSize: '1.4rem', color: isLive ? 'var(--green)' : 'var(--accent)' }}>
      ...
      {/* Away */}
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', fontSize: '0.875rem', fontWeight: 600, justifyContent: 'flex-end' }}>
        <span>{away}</span>
        <span style={{ fontSize: '1.2rem' }}>{aFlag}</span>
      </div>
    </div>
  )
}
          ...
          {nextMatch && (
            <div style={{ textAlign: 'right' }}>
              ...
              <div style={{ fontSize: '0.8rem', fontWeight: 600, marginBottom: '0.4rem' }}>
                {FLAGS[nextMatch.home] || '🏳️'} {nextMatch.home} vs {nextMatch.away} {FLAGS[nextMatch.away] || '🏳️'}
              </div>
